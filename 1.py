import urllib.request
import urllib.parse
import re
urlArray = []#存已经找的网址
def findURL(urlname):
    global urlArray#global代表这个变量是全局变量
    if urlname in urlArray:#判断是否这个网页是否在数组中，如果在，代表已经找过，直接返回
        return
    print(urlname)#输出当前的网址
    urlArray.append(urlname)#然后把这个网址放进数组
    req = urllib.request.Request(urlname)#建立连接，获取网页源码
    req.add_header('User-Agent','Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 ''(KHTML, like Gecko) Chrome/52.0.2743.116 Safari/537.36')
    response = urllib.request.urlopen(req)
    html = response.read().decode('utf-8')
    urls = re.findall(r"http://[^\"\'\n;><)]+",html,re.I)#正则表达式挑出里面的网址
    html=[]#为了释放html占的资源（不知道行不行的通）
    for i in urls:#遍历找到的所有的网址
         print(i)#输出网址
         #findURL(i)#
    return
#主程序
urlName="http://www.baidu.com/"#要爬取得第一个网页ZJ
findURL(urlName)#调用自己的函数
print("已经找完了！\n")#111